...
namespace Microsoft.Reporting.Map.WebForms
{
	...
	internal class Symbol : NamedElement, IContentElement, ILayerElement, IToolTipProvider, ISelectable, ISpatialElement, IImageMapProvider
	{
		...
		public bool AddGeometry(SqlGeometry geometry)
		{
			...
			AddGeometryRec(geometry, arrayList);
			if (arrayList.Count > 0)
			{
				SetPoints((MapPoint[])arrayList.ToArray(typeof(MapPoint)));
				return true;
			}
			return false;
		}
		...
		private void AddGeometryRec(SqlGeometry geometry, ArrayList pointsList)
		{
			...
			case "GeometryCollection":
			{
				...
				{
					AddGeometryRec(geometry.STGeometryN(i), pointsList);
				}
				break;
			}
			case "MultiPoint":
			case "Point":
				AddSimpleGeometry(geometry, pointsList);
				break;
			}
		}
		...
		public bool AddGeography(SqlGeography geography)
		{
			...
			ArrayList arrayList = new ArrayList();
			AddGeographyRec(geography, arrayList);
			if (arrayList.Count > 0)
			{
				SetPoints((MapPoint[])arrayList.ToArray(typeof(MapPoint)));
				return true;
		...
		private void AddGeographyRec(SqlGeography geography, ArrayList pointsList)
		{
			...
			case "GeometryCollection":
			{
				...
				{
					AddGeographyRec(geography.STGeometryN(i), pointsList);
				}
				break;
			}
			case "MultiPoint":
			case "Point":
				AddSimpleGeography(geography, pointsList);
				break;
			}
		}
