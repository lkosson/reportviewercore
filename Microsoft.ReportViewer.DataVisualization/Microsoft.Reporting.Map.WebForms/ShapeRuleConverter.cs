...
{
	internal class ShapeRuleConverter : CollectionItemTypeConverter
	{
		public ShapeRuleConverter()
		{
			simpleType = typeof(ShapeRule);
		}
		...
		public override PropertyDescriptorCollection GetProperties(ITypeDescriptorContext context, object value, Attribute[] attributes)
		{
			...
			for (int i = 0; i < properties.Count; i++)
			{
				...
				else if ((properties[i].Name == "FromColor" || properties[i].Name == "MiddleColor" || properties[i].Name == "ToColor") && shapeRule.ColoringMode == ColoringMode.DistinctColors)
				{
					propertyDescriptorCollection.Add(TypeDescriptor.CreateProperty(value.GetType(), properties[i], new ReadOnlyAttribute(isReadOnly: true)));
				}
				else if (properties[i].Name == "CustomColors" && !shapeRule.UseCustomColors)
				{
