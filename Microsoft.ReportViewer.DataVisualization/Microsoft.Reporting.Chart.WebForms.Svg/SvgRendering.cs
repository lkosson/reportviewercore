...
namespace Microsoft.Reporting.Chart.WebForms.Svg
{
	internal class SvgRendering : ChartParameters
	{
		...
		public void Open(XmlTextWriter svgWriter, Size pictureSize, SvgOpenParameters extraParameters)
		{
			...
			output.WriteComment(SR.MessageSvgConverterTitle);
			output.WriteStartElement("svg", "http://www.w3.org/2000/svg");
			output.WriteAttributeString("xmlns:xlink", "http://www.w3.org/1999/xlink");
			if (extraParameters.Resizable)
			{
				...
				output.WriteAttributeString("viewBox", "0 0 " + base.PictureSize.Width.ToString(CultureInfo.InvariantCulture) + " " + base.PictureSize.Height.ToString(CultureInfo.InvariantCulture));
			...
			{
				output.WriteAttributeString("width", base.PictureSize.Width.ToString(CultureInfo.InvariantCulture));
				output.WriteAttributeString("height", base.PictureSize.Height.ToString(CultureInfo.InvariantCulture));
			}
		...
		private void SetPie(RectangleF rect, float startAngle, float sweepAngle)
		{
			...
			GraphicsPath graphicsPath = new GraphicsPath();
			graphicsPath.AddArc(rect, startAngle, sweepAngle);
			if (!(sweepAngle <= 0f) && graphicsPath.PointCount != 0)
			{
				output.WriteStartElement("path");
				...
				empty = empty + " a" + GetX(rect.Width / 2f) + "," + GetY(rect.Height / 2f);
				empty = empty + " 0," + num + ",1 ";
				empty = empty + GetX(pointF2.X - pointF.X) + "," + GetY(pointF2.Y - pointF.Y);
		...
		public void DrawImage(Image image, Rectangle destinationRect, int sourceX, int sourceY, int sourceWidth, int sourceHeight, GraphicsUnit sourceUnit, ImageAttributes imageAttr)
		{
			...
			Graphics.FromImage(image2).DrawImage(image, new Rectangle(0, 0, destinationRect.Width, destinationRect.Height), sourceX, sourceY, sourceWidth, sourceHeight, sourceUnit, imageAttr);
			output.WriteStartElement("image");
			string str = ImageToString(image2);
			output.WriteAttributeString("xlink:href", "data:image/jpeg;base64," + str);
			output.WriteAttributeString("x", GetX(destinationRect));
			output.WriteAttributeString("y", GetY(destinationRect));
			output.WriteAttributeString("width", GetWidth(destinationRect));
			output.WriteAttributeString("height", GetHeight(destinationRect));
			output.WriteEndElement();
			image2.Dispose();
		}
		...
		public void DrawImage(Image image, RectangleF dstRect)
		{
			...
			Graphics.FromImage(image2).DrawImage(image, new RectangleF(0f, 0f, dstRect.Width, dstRect.Height));
			output.WriteStartElement("image");
			string str = ImageToString(image2);
			output.WriteAttributeString("xlink:href", "data:image/jpeg;base64," + str);
			output.WriteAttributeString("x", GetX(dstRect));
			output.WriteAttributeString("y", GetY(dstRect));
			output.WriteAttributeString("width", GetWidth(dstRect));
			output.WriteAttributeString("height", GetHeight(dstRect));
			output.WriteEndElement();
			image2.Dispose();
		}
		...
		public void DrawImage(Image image, Rectangle destinationRect, float sourceX, float sourceY, float sourceWidth, float sourceHeight, GraphicsUnit sourceUnit, ImageAttributes imageAttributes)
		{
			Validate();
			Image image2 = new Bitmap(image, destinationRect.Width, destinationRect.Height);
			Graphics.FromImage(image2).DrawImage(image, new Rectangle(0, 0, destinationRect.Width, destinationRect.Height), sourceX, sourceY, sourceWidth, sourceHeight, sourceUnit, imageAttributes);
			output.WriteStartElement("image");
			string str = ImageToString(image2);
			output.WriteAttributeString("xlink:href", "data:image/jpeg;base64," + str);
			output.WriteAttributeString("x", GetX(destinationRect));
		...
		public void FillTexturedRectangle(TextureBrush textureBrush, RectangleF dstRect)
		{
			...
			Graphics.FromImage(image).FillRectangle(textureBrush, dstRect);
			output.WriteStartElement("image");
		...
		public void DrawString(string text, RectangleF layoutRect)
		{
			...
			int num3 = stringArray.Length - 1;
			while (num3 >= 0 && (stringArray[num3] == null || stringArray[num3].Length == 0))
			{
				num2--;
				num3--;
			}
			...
			foreach (string text2 in array)
			{
				...
				int num4 = (int)Math.Round((double)Font.SizeInPoints * (Chart.renderingDpiY / 72.0));
				...
				{
					num6 += (float)num4 * 0.75f;
					num6 += (layoutRect.Height - (float)(num2 * num4)) / 2f;
				}
				output.WriteStartElement("text");
				output.WriteAttributeString("x", ToUSString(number));
				output.WriteAttributeString("y", ToUSString(y + num6 + num5));
		...
		public float WrapString(string text, RectangleF dstRectangle, out string[] stringArray)
		{
			if (stringGraph == null)
			{
				stringBitmap = new Bitmap(base.PictureSize.Width, base.PictureSize.Height);
				stringGraph = Graphics.FromImage(stringBitmap);
			}
			SizeF sizeF = stringGraph.MeasureString(text, Font, base.PictureSize.Width, StringFormat);
			dstRectangle.Height = base.PictureSize.Height;
			...
			foreach (string text2 in array2)
			{
				...
				sizeF = stringGraph.MeasureString(empty, Font, base.PictureSize.Width * 10, StringFormat);
		...
		internal void StartGraphicsParameters(bool fill, bool outline)
		{
			...
			if (clipSet)
			{
				output.WriteAttributeString("clip-path", "url(#ChartClipID" + clipRegionIdNum + ")");
			}
			if (fill)
			{
			...
			if (outline)
			{
				...
				if (GetDashStyle(DashStyle).Length != 0)
				{
		...
		private string SetGradient(Color firstColor, Color secondColor, SvgGradientType type)
		{
			...
			{
				throw new InvalidOperationException(SR.ExceptionSvgMaximumGradientsNumberExceeded(int.MaxValue.ToString(CultureInfo.CurrentCulture)));
			}
			switch (type)
			{
		...
		private void SetRadialGradient(string gradientID, Color firstColor, Color secondColor)
		{
			...
			output.WriteAttributeString("stop-opacity", GetAlpha(firstColor));
			output.WriteEndElement();
			output.WriteStartElement("stop");
			output.WriteAttributeString("offset", "100%");
			output.WriteAttributeString("stop-color", ColorToString(secondColor));
			output.WriteAttributeString("stop-opacity", GetAlpha(secondColor));
			output.WriteEndElement();
			output.WriteEndElement();
			output.WriteEndElement();
		}
		...
		public void SetClip(RectangleF rect)
		{
			...
			output.WriteAttributeString("id", "ChartClipID" + clipRegionIdNum);
