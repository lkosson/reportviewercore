    ...
    internal class ImageInformation
    {
        ...
        private void CalculateMetrics()
        {
            if (m_imageData != null && m_imageData.Length != 0L)
            {
                ...
                var image = SixLabors.ImageSharp.Image.Load(m_imageData, out var format);
                // System.Drawing.Image image = System.Drawing.Image.FromStream(m_imageData);
                m_imageFormat = ToDrawingFormat(format);
                Width = image.Width;
                Height = image.Height;
        ...
        public static ImageFormat ToDrawingFormat(SixLabors.ImageSharp.Formats.IImageFormat format)
        {
            switch (format)
            {
                case SixLabors.ImageSharp.Formats.Png.PngFormat png:
                    return ImageFormat.Png;
                case SixLabors.ImageSharp.Formats.Jpeg.JpegFormat jpg:
                    return ImageFormat.Jpeg;
                case SixLabors.ImageSharp.Formats.Bmp.BmpFormat bmp:
                    return ImageFormat.Bmp;
                default:
                    throw new System.Exception("Unkown format");
            };
        }
